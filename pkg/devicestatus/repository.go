...
type IRepository interface {
	Insert(entries []entities.DeviceStatus) error
	Find(
		ctx context.Context,
		spec *common.QuerySpec,
	) ([]*entities.DeviceStatus, error)
	GetOne(ctx context.Context, spec *common.QuerySpec) (*entities.DeviceStatus, error)
...
func (r *Repository) Insert(entries []entities.DeviceStatus) error {
	for i := range entries {
...
func (r *Repository) GetOne(ctx context.Context, spec *common.QuerySpec) (*entities.DeviceStatus, error) {
	q := r.db.NewSelect().Model((*entities.DeviceStatus)(nil))
	for _, f := range spec.Filters {
		q = q.Where("? "+string(f.Op)+" ?", bun.Ident(f.Field), f.Value)
	}
	var entries []entities.DeviceStatus
